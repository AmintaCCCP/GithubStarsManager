...
| # | Contract | Where | Why |
|---|----------|-------|-----|
| 1 | `discoveryRepos` is **never persisted** | `partialize` omits it (`options.ts:132` comment + omitted key) | It is an extremely large JSON object. Re-fetching on load is cheaper than rehydrating megabytes from IndexedDB. Migrate must never add it to `partialize`. |
