...
const RepositoryCardComponent: React.FC<RepositoryCardProps> = ({
  ...
  const cardClassName = useMemo(() => {
    const baseClasses = 'repository-card group bg-white dark:bg-panel-dark rounded-xl border border-gray-200 dark:border-white/8 p-6 hover:shadow-subtle transition-shadow transition-colors duration-200 hover:border-blue-300 dark:hover:border-white/20 flex flex-col h-full cursor-pointer select-none';
    ...
    >
        ...
        {!selectionMode && (
          <div className="hidden lg:block relative flex-shrink-0 opacity-0 hover:opacity-100 transition-opacity duration-200 group-hover:opacity-100">
            <div
              ...
              className="flex items-center justify-center w-8 h-8 rounded-lg cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-600 dark:text-text-tertiary dark:hover:text-text-primary hover:bg-gray-100 dark:hover:bg-white/5 transition-all duration-200 touch-manipulation"
            ...
            {/* 弱气泡提示 */}
            {showDragHint && (
              <div className="absolute top-full right-0 mt-2 px-3 py-1.5 bg-gray-800 dark:bg-surface-3 text-white dark:text-text-primary text-xs rounded-lg shadow-dialog dark:border dark:border-white/8 whitespace-nowrap z-50 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          <SelectionAwareButton
            ...
            className={`${
              ...
              : repository.analyzed_at
                ? 'bg-green-100 text-green-600 dark:bg-status-emerald/20 dark:text-status-emerald hover:bg-green-200 dark:hover:bg-status-emerald/30'
          ...
          <SelectionAwareButton
            ...
            className={`${isSubscribed
              ? 'bg-blue-100 text-blue-600 dark:bg-brand-indigo/20 dark:text-brand-violet'
              : 'bg-gray-100 text-gray-500 dark:bg-white/[0.04] dark:text-text-secondary hover:bg-gray-200 dark:hover:bg-white/10'
            }`}
        ...
        <div className="flex items-center space-x-2">
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={(e) => selectionMode && e.preventDefault()}
            className={`flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 text-gray-500 dark:bg-white/[0.04] dark:text-text-secondary hover:bg-gray-200 dark:hover:bg-white/10 transition-colors ${selectionMode ? 'pointer-events-none opacity-50' : ''}`}
      ...
      <div className="mb-4 flex-1">
        ...
        >
          <p
            ref={descriptionRef}
            className="text-gray-700 dark:text-text-secondary text-sm leading-relaxed line-clamp-3 mb-2"
          >
          ...
          {isTextTruncated && showTooltip && (
            <div className="absolute z-50 bottom-full left-0 right-0 mb-2 p-3 bg-gray-900 dark:bg-surface-3 text-white dark:text-text-primary text-sm rounded-lg shadow-dialog border border-gray-200 dark:border-white/8 max-h-48 overflow-y-auto">
          ...
          <div className="flex space-x-1">
            {repository.ai_platforms.slice(0, 6).map((platform, index) => {
              ...
              return (
                <div
                  key={index}
                  className="w-6 h-6 flex items-center justify-center bg-gray-100 dark:bg-white/[0.04] rounded text-gray-600 dark:text-text-tertiary hover:bg-gray-200 dark:hover:bg-white/10 dark:hover:text-text-secondary transition-colors cursor-default"
                  title={displayName}
                >
      ...
      <div className="space-y-3 mt-auto">
        ...
        <div className="flex items-center justify-between text-sm text-gray-500 dark:text-text-tertiary pt-2 border-t border-gray-100 dark:border-white/8">
          <div className="flex items-center space-x-1">
          ...
          {onSelect && (
            <button
              ...
              className={`flex items-center justify-center w-8 h-8 rounded-lg transition-colors ${
                isSelected
                  ? 'bg-blue-100 text-blue-600 dark:bg-brand-indigo/20 dark:text-brand-violet'
                  : 'text-gray-400 dark:text-text-tertiary hover:bg-gray-100 dark:hover:bg-white/10'
              }`}
