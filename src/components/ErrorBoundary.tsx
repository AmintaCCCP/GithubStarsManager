...
export class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-screen bg-gray-50 dark:bg-panel-dark flex items-center justify-center p-4">
          <div className="max-w-lg w-full bg-white dark:bg-panel-dark rounded-lg shadow-lg p-6">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-text-tertiary mb-4">
                {strings.description}
              </p>
              ...
              <div className="mb-4">
                <button
                  ...
                  className="text-sm text-blue-600 dark:text-brand-violet hover:text-blue-800 dark:hover:text-blue-300 underline"
                >
                  {strings.toggleDetails}
                </button>
                {showDetails && errorInfo && (
                  <div className="mt-2 p-3 bg-gray-100 dark:bg-white/[0.04] rounded text-left overflow-auto max-h-64">
                    <p className="text-xs font-semibold text-gray-700 dark:text-text-secondary mb-2">
                      {strings.stackTrace}:
                    </p>
                    <pre className="text-xs text-gray-600 dark:text-text-tertiary font-mono whitespace-pre-wrap">
                      {error?.stack || 'No stack trace available'}
                    </pre>
                    {errorInfo?.componentStack && (
                      <>
                        <p className="text-xs font-semibold text-gray-700 dark:text-text-secondary mt-3 mb-2">
                          Component Stack:
                        </p>
                        <pre className="text-xs text-gray-600 dark:text-text-tertiary font-mono whitespace-pre-wrap">
                          {errorInfo.componentStack}
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full px-4 py-2 bg-gray-100 text-gray-700 dark:bg-white/[0.04] dark:text-text-secondary rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
                >
                  {strings.reportIssue}
                </button>
              </div>
