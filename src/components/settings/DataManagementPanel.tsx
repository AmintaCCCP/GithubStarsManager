...
export const DataManagementPanel: React.FC<DataManagementPanelProps> = ({ t }) => {
  ...
  const cleanupSuggestions = useMemo<DataCleanupSuggestion[]>(() => {
    ...
    if (unanalyzedRepos > 10) {
      suggestions.push({
        ...
        bgColor: 'bg-blue-50 dark:bg-brand-indigo/20/20'
      });
    }
    ...
    if (oldReadReleases > 50) {
      suggestions.push({
        ...
        color: 'text-status-emerald dark:text-green-400',
        bgColor: 'bg-status-emerald dark:bg-green-900/20'
      });
    }
  ...
  const dataStats = [
    ...
    },
    {
      ...
      color: 'text-text-secondarydark:text-text-tertiary',
      bgColor: 'bg-light-bgdark:bg-panel-dark/20',
    },
  ];
  ...
  return (
    <div className="space-y-8">
      {/* Success Message */}
      {showSuccessMessage && (
        <div className="fixed top-4 right-4 z-50 flex items-center space-x-2 px-4 py-3 bg-status-emerald dark:bg-green-900/30 text-status-emerald dark:text-green-200 rounded-lg shadow-lg animate-in slide-in-from-top-2">
      ...
      {/* Data Statistics */}
      <section>
        <h3 className="text-lg font-semibold text-text-primarydark:text-text-primary mb-4 flex items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {dataStats.map((stat) => (
            <div
              key={stat.key}
              className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 p-4"
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm text-text-tertiarydark:text-text-tertiary">{stat.label}</p>
                    <p className="text-2xl font-bold text-text-primarydark:text-text-primary">
                      {stat.count}
                    </p>
                  </div>
                </div>
      ...
      <section>
        <h3 className="text-lg font-semibold text-text-primarydark:text-text-primary mb-4 flex items-center">
          <HardDrive className="w-5 h-5 mr-2 text-status-emerald dark:text-green-400" />
          {t('数据导出与导入', 'Data Export & Import')}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Export */}
          <div className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 p-4">
            <div className="flex items-center space-x-3 mb-4">
              <div className="p-2 rounded-lg bg-blue-50 dark:bg-brand-indigo/20/20 text-brand-violet dark:text-brand-violet">
                <Download className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-medium text-text-primarydark:text-text-primary">{t('导出数据', 'Export Data')}</h4>
                <p className="text-sm text-text-tertiarydark:text-text-tertiary">{t('将数据导出为JSON文件', 'Export data to JSON file')}</p>
              </div>
            </div>
            <div className="space-y-2 mb-4">
              {[
                ...
                <label key={item.key} className="flex items-center space-x-2 text-sm text-text-primarydark:text-text-secondary">
                  <input
                    type="checkbox"
                    defaultChecked
                    className="export-checkbox rounded border-light-border dark:border-white/12 text-brand-violet focus:ring-brand-violet"
                    data-type={item.key}
                  />
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-brand-indigo hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
            >
              {isExporting ? (
                <>
          ...
          <div className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 p-4">
            <div className="flex items-center space-x-3 mb-4">
              <div className="p-2 rounded-lg bg-status-emerald dark:bg-green-900/20 text-status-emerald dark:text-green-400">
                <Upload className="w-5 h-5" />
              </div>
              <div>
                <h4 className="font-medium text-text-primarydark:text-text-primary">{t('导入数据', 'Import Data')}</h4>
                <p className="text-sm text-text-tertiarydark:text-text-tertiary">{t('从JSON文件导入数据', 'Import data from JSON file')}</p>
              </div>
            </div>
            <div className="border-2 border-dashed border-light-border dark:border-white/12 rounded-lg p-6 text-center">
              <Upload className="w-8 h-8 text-gray-400 mx-auto mb-2" />
              <p className="text-sm text-text-tertiarydark:text-text-tertiary mb-2">
              ...
              <label
                ...
                className="cursor-pointer px-4 py-2 bg-light-surfacedark:bg-white/[0.04] hover:bg-gray-200 dark:hover:bg-gray-600 text-text-primarydark:text-text-secondary rounded-lg transition-colors inline-block"
              >
                {t('选择文件', 'Select File')}
              </label>
            </div>
        ...
        <section>
          ...
          <div className="space-y-3">
            {cleanupSuggestions.map((suggestion) => (
              <div
                key={suggestion.key}
                className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 p-4 flex items-center justify-between"
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-medium text-text-primarydark:text-text-primary">
                ...
                <div className="flex items-center space-x-3">
                  <span className="text-sm font-medium text-text-secondarydark:text-text-tertiary">
      ...
      <section>
        ...
        <div className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 overflow-hidden">
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {dataStats.map((stat) => (
              <div
                key={stat.key}
                className="flex items-center justify-between px-4 py-4 hover:bg-light-bgdark:hover:bg-white/10/50 transition-colors"
              >
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg ${stat.bgColor} ${stat.color}`}>
                    {stat.icon}
                  ...
                  <div>
                    <p className="font-medium text-text-primarydark:text-text-primary">{stat.label}</p>
                    <p className="text-sm text-text-tertiarydark:text-text-tertiary">
      ...
      {operationLogs.length > 0 && (
        <section>
          <h3 className="text-lg font-semibold text-text-primarydark:text-text-primary mb-4">
            {t('操作日志', 'Operation Logs')}
          </h3>
          <div className="bg-white dark:bg-panel-dark rounded-lg border border-light-border dark:border-white/12 overflow-hidden">
            <div className="max-h-64 overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="bg-light-bgdark:bg-white/[0.04] sticky top-0">
                  <tr>
                    <th className="px-4 py-2 text-left text-text-secondarydark:text-text-secondary">
                      {t('时间', 'Time')}
                    </th>
                    <th className="px-4 py-2 text-left text-text-secondarydark:text-text-secondary">
                      {t('操作', 'Operation')}
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {operationLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-light-bgdark:hover:bg-white/10/50">
                      <td className="px-4 py-2 text-text-tertiarydark:text-text-tertiary">
                        {log.timestamp}
                      </td>
                      <td className="px-4 py-2 text-text-primarydark:text-text-primary">{log.operation}</td>
                      <td className="px-4 py-2">
                        {log.success ? (
                          <span className="inline-flex items-center px-2 py-1 text-xs font-medium text-status-emerald dark:text-green-300 bg-status-emerald dark:bg-green-900/30 rounded-full">
            ...
            <div className="p-6 space-y-4">
              ...
              {confirmation.type === 'all' && user && (
                <div className="space-y-2">
                  <label className="block text-sm font-medium text-text-primarydark:text-text-secondary">
                    {t(
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2 border border-light-border dark:border-white/12 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 dark:bg-white/[0.04] dark:text-text-primary"
                  />
                </div>
              )}
              ...
              <div className="flex space-x-3 pt-4">
                <button
                  ...
                  disabled={isDeleting}
                  className="flex-1 px-4 py-2 text-text-primarydark:text-text-secondary bg-light-surfacedark:bg-white/[0.04] hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors disabled:opacity-50"
                >
                  {t('取消', 'Cancel')}
                </button>
                <button
      ...
      {importPreview.isOpen && importPreview.data && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white dark:bg-panel-dark rounded-2xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 bg-status-emerald dark:bg-green-900/20 border-b border-green-100 dark:border-green-800">
              <div className="flex items-center space-x-3">
                <Upload className="w-6 h-6 text-status-emerald dark:text-green-400" />
            ...
            <div className="p-6 space-y-4">
              <div className="text-sm text-text-secondarydark:text-text-tertiary">
              ...
              <div className="border-t border-light-border dark:border-white/12 pt-4">
                <p className="text-sm font-medium text-text-primarydark:text-text-secondary mb-2">{t('包含的数据:', 'Included Data:')}</p>
                <div className="space-y-1 text-sm">
                  {importPreview.data.data.repositories && (
                    <p className="text-text-secondarydark:text-text-tertiary">
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  ...
                  disabled={isImporting}
                  className="flex-1 px-4 py-2 bg-brand-indigo hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {isImporting ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                ...
                <button
                  ...
                  disabled={isImporting}
                  className="flex-1 px-4 py-2 bg-brand-indigo hover:bg-green-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {isImporting ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
