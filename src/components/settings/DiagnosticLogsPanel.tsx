...
function getHttpSummary(data: unknown): string | null {
  if (!data || typeof data !== 'object' || Array.isArray(data)) return null;
  const d = data as Record<string, unknown>;
  if (!d.method && !d.status && !d.durationMs) return null;
  const parts: string[] = [];
  if (d.method) parts.push(String(d.method));
  if (d.endpoint || d.path) parts.push(String(d.endpoint ?? d.path));
  if (d.status) {
...
function getStatusColor(status: unknown): string {
  ...
  return STATUS_COLORS[firstDigit] || '';
}
...
export const DiagnosticLogsPanel: React.FC<DiagnosticLogsPanelProps> = ({ t }) => {
  ...
  const availableEventTypes = useMemo(() => {
    ...
    for (const entry of allEntries) {
      types.add(inferEventType(entry.module, entry.message));
    }
  ...
  const handleClear = useCallback(async () => {
    if (selectedScope === 'frontend' || selectedScope === 'all') {
      logger.clear();
      setEntries([]);
    }
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const exportData = {
        ...
        environment,
        sanitizationNote: t(
          '所有 Token、API Key、密码、邮箱已脱敏为 ***格式',
          'All tokens, API keys, passwords, and emails have been masked as ***<last4>'
        ),
        frontendLogs,
        backendLogs,
      };
  ...
  // Toggle event type
  const toggleEventType = useCallback((eventType: LogEventType) => {
  ...
  const toggleDataExpand = useCallback((id: string) => {
    ...
    <div className="space-y-4">
        ...
        <div className="bg-white dark:bg-panel-dark rounded-lg border border-black/[0.06] dark:border-white/[0.04] p-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${frontendDebug ? 'text-green-600 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'}`}
              aria-label={frontendDebug ? t('关闭前端调试', 'Disable frontend debug') : t('开启前端调试', 'Enable frontend debug')}
            >
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <div className="flex items-center space-x-2">
                <span className={`font-medium ${backendAvailable ? 'text-gray-900 dark:text-text-primary' : 'text-gray-400 dark:text-text-quaternary'}`}>
                  {t('后端调试', 'Backend Debug')}
                </span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${backendDebug ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-white/[0.06] dark:text-gray-400'}`}>
                  {backendAvailable
                    ? (backendDebug ? t('已开启', 'ON') : t('已关闭', 'OFF'))
                    : t('后端未连接', 'Not connected')}
                </span>
              </div>
              <p className="text-sm text-gray-500 dark:text-text-tertiary mt-1">
            ...
            <button
              onClick={backendAvailable ? toggleBackendDebug : undefined}
              ...
              aria-label={backendDebug ? t('关闭后端调试', 'Disable backend debug') : t('开启后端调试', 'Enable backend debug')}
            >
      ...
      <section className="bg-white dark:bg-panel-dark rounded-lg border border-black/[0.06] dark:border-white/[0.04] p-4 space-y-3">
        ...
        <div className="flex items-center space-x-2">
          ...
          {(['debug', 'info', 'warn', 'error'] as LogLevel[]).map(level => {
            ...
            return (
              <button
                key={level}
                onClick={disabled ? undefined : () => toggleLevel(level)}
                disabled={disabled}
        ...
        <div className="flex items-center space-x-3 flex-wrap gap-y-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 text-sm transition-colors ${
                  selectedScope === scope
                    ...
                    : 'bg-transparent text-gray-600 dark:text-text-secondary hover:bg-gray-100 dark:hover:bg-white/[0.06]'
                } ${scope === 'backend' && !backendAvailable ? 'opacity-40 cursor-not-allowed' : ''}`}
              >
                ...
                {availableEventTypes.map(et => (
                  <button
                    ...
                    className={`w-full text-left px-2 py-1 text-sm rounded flex items-center space-x-2 ${
                      selectedEventTypes.has(et)
                        ? 'bg-brand-indigo/10 text-brand-indigo dark:bg-brand-violet/20 dark:text-brand-violet'
                        : 'text-gray-700 dark:text-text-secondary hover:bg-gray-100 dark:hover:bg-white/[0.06]'
                    }`}
                  >
          ...
          <div className="flex items-center space-x-2 ml-auto">
            <button
              ...
              disabled={isRefreshing || !backendAvailable}
            ...
            <button
              onClick={handleClear}
              className="px-3 py-1.5 text-sm font-medium rounded-lg text-gray-700 dark:text-text-secondary bg-gray-100 dark:bg-white/[0.04] hover:bg-gray-200 dark:hover:bg-white/[0.08] transition-colors flex items-center space-x-1"
            >
        ...
        <div className="text-xs text-gray-500 dark:text-text-tertiary">
          ...
          {(frontendDebug || backendDebug) && (
            <span className="ml-2 text-amber-600 dark:text-amber-400">
              {t('调试模式已开启', 'Debug mode ON')}
            </span>
          )}
          {selectedScope !== 'backend' && (
            <span className="ml-1">
              · {t(`前端 ${frontendCounts.total}`, `Frontend ${frontendCounts.total}`)}
            </span>
          )}
          {selectedScope !== 'frontend' && backendAvailable && (
            <span className="ml-1">
              · {t(`后端 ${backendLogCount}`, `Backend ${backendLogCount}`)}
            </span>
          )}
        </div>
      </section>
        ...
        {filteredEntries.length === 0 ? (
          ...
          <div className="max-h-[520px] overflow-y-auto divide-y divide-black/[0.04] dark:divide-white/[0.02]">
            {filteredEntries.map(entry => {
              ...
              const httpSummary = getHttpSummary(entry.data);
              const entryData = entry.data as Record<string, unknown> | undefined;
              const hasData = entryData && typeof entryData === 'object' && !Array.isArray(entryData) && Object.keys(entryData).length > 0;
              const isExpanded = expandedData.has(entry.id);
              const statusColor = entryData?.status ? getStatusColor(entryData.status) : '';

              return (
                <div key={entry.id} className="px-4 py-3 hover:bg-light-surface dark:hover:bg-white/[0.02] transition-colors">
                  {/* Header row */}
                  <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                    ...
                    <span
                      className="text-xs text-gray-500 dark:text-text-tertiary"
                  ...
                  {/* HTTP summary line */}
                  {httpSummary && (
                    <div className="text-xs mt-1 font-mono flex items-center space-x-1">
                      <span className="font-bold">{String(entryData?.method ?? '')}</span>
                      <span className="text-gray-600 dark:text-text-secondary">{String(entryData?.endpoint ?? entryData?.path ?? '')}</span>
                      {entryData?.status && (
                        <span className={`font-bold ${statusColor}`}>→ {String(entryData.status)}</span>
                      )}
                      {entryData?.durationMs && (
                        <span className="text-blue-600 dark:text-blue-400">{String(entryData.durationMs)}ms</span>
                      )}
                    </div>
                  )}

                  {/* Expandable data block */}
                  {hasData && (
                    <div className="mt-1">
                      <button
                        onClick={() => toggleDataExpand(entry.id)}
                        className="text-xs text-gray-500 dark:text-text-tertiary hover:text-gray-700 dark:hover:text-text-secondary flex items-center space-x-1"
                      >
                        {isExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                        <span>{isExpanded ? t('收起详情', 'Hide details') : t('查看详情', 'Show details')}</span>
                      </button>
                      {isExpanded && (
                        <pre className="mt-1 text-xs bg-gray-50 dark:bg-white/[0.02] rounded p-2 overflow-auto max-h-48 font-mono text-gray-700 dark:text-text-secondary">
