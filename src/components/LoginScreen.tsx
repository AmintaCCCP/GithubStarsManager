...
import { Github, Key, ArrowRight, AlertCircle, Moon, Sun, Star, ListChecks } from 'lucide-react';
...
export const LoginScreen: React.FC = () => {
  ...
  const [showSyncModeChoice, setShowSyncModeChoice] = useState(false);
  const { setUser, setGitHubToken, setSyncMode, setSyncModeConfigured, repositories, lastSync, language, setLanguage, theme, setTheme } = useAppStore();
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      // 首次登录：若从未设置过同步范围，弹出选择框（选择结果持久化，可在设置中切换）
      const store = useAppStore.getState();
      if (!store.syncModeConfigured) {
        setShowSyncModeChoice(true);
      }
    } catch (error) {
