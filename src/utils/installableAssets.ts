...
const EXCLUSION_RULES: ReadonlyArray<{ reason: string; pattern: RegExp }> = [
  ...
  { reason: 'debug symbols', pattern: /\.(?:pdb|dsym)$|(?:^|[^a-z0-9])(?:symbols?|debug|dsym)(?=$|[^a-z0-9])/ },
...
export function detectInstallableAssets(
  ...
  for (const asset of assets ?? []) {
    ...
    if (deviceArchitecture && architecture && architecture !== 'universal' && architecture !== deviceArchitecture) {
      excluded.push({
        assetId: asset.id,
        fileName,
    ...
    if (fromExtension && architecture && !ambiguous) confidence = 'high';
    else if (fromExtension) confidence = 'medium';
    else if (architecture && !ambiguous) confidence = 'medium';
    else confidence = 'low';
