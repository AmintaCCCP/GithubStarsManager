const ERROR_MESSAGES: Record<string, { zh: string; en: string }> = {
  ...
  SYNC_AI_CONFIGS_ALL_SKIPPED: { zh: '同步 AI 配置失败：所有配置均被跳过，请检查 API Key 是否正确填写', en: 'Failed to sync AI configs: all configs skipped, please check API Keys' },
  SYNC_WEBDAV_CONFIGS_FAILED: { zh: '同步 WebDAV 配置失败', en: 'Failed to sync WebDAV configs' },
  SYNC_WEBDAV_CONFIGS_ALL_SKIPPED: { zh: '同步 WebDAV 配置失败：所有配置均被跳过，请检查密码是否正确填写', en: 'Failed to sync WebDAV configs: all configs skipped, please check passwords' },
