...
所有角色镜像均使用相同的标签语义：`latest` 表示 `main` 的最新构建；`v0.8.1` 表示与客户端完全一致的正式发布标签；`0.8.1`、`0.8`、`0` 是由该正式标签派生的便捷标签；`sha-abc1234` 表示指定提交。根目录 `package.json` 的 `version` 是客户端和 Docker 正式发布的唯一版本来源；只有与该版本完全匹配的 `v<version>` Git 标签才能发布正式镜像。发布镜像同时包含 `linux/amd64` 与 `linux/arm64` 变体，Docker 会根据宿主机架构自动选择 x86_64 或 ARM64 版本。
